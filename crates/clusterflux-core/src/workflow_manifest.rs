...
pub const SUPPORTED_WORKFLOW_SDK_VERSION: &str = "0.1.0";
...
mod tests {
    ...
    fn valid_manifest(name: &str) -> String {
        format!(
            "[package]\nname = \"{name}\"\nversion = \"0.0.0\"\nedition = \"2024\"\npublish = false\n\n[lib]\npath = \"main.rs\"\ncrate-type = [\"cdylib\"]\n\n[dependencies]\nclusterflux = {{ package = \"clusterflux-sdk\", version = \"=0.1.0\", path = \"../crates/clusterflux-sdk\" }}\n\n[workspace]\nresolver = \"3\"\n"
        )
    }
    ...
    fn rejects_wrong_exact_values_and_resource_excess() {
        let wrong_sdk = valid_manifest("demo").replace("=0.1.0", "=0.2.0");
        assert!(matches!(
